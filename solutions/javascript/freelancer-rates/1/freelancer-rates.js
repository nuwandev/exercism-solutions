...
export function priceWithMonthlyDiscount(ratePerHour, numDays, discount) {
  ...
  const extraDays = numDays % daysPerMonth;

  const fullMonthPrice = fullMonths * daysPerMonth * dayRate;
  const discountedMonthPrice = fullMonthPrice * (1 - discount);

  const extraDaysPrice = extraDays * dayRate;

  const total = discountedMonthPrice + extraDaysPrice;
